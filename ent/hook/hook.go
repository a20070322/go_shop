...
// The WeChatPayFunc type is an adapter to allow the use of ordinary
// function as WeChatPay mutator.
type WeChatPayFunc func(context.Context, *ent.WeChatPayMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WeChatPayFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.WeChatPayMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WeChatPayMutation", m)
	}
	return f(ctx, mv)
}
