...
func (tx *Tx) init() {
	tx.BasicBanner = NewBasicBannerClient(tx.config)
	tx.BasicBannerPosition = NewBasicBannerPositionClient(tx.config)
	tx.BasicLink = NewBasicLinkClient(tx.config)
	tx.Customer = NewCustomerClient(tx.config)
	tx.CustomerAddress = NewCustomerAddressClient(tx.config)
	tx.GoodsClassify = NewGoodsClassifyClient(tx.config)
	tx.GoodsSku = NewGoodsSkuClient(tx.config)
	tx.GoodsSpecs = NewGoodsSpecsClient(tx.config)
	tx.GoodsSpecsOption = NewGoodsSpecsOptionClient(tx.config)
	tx.GoodsSpu = NewGoodsSpuClient(tx.config)
	tx.GoodsSpuImgs = NewGoodsSpuImgsClient(tx.config)
...
// applies a query, for example: BasicBanner.QueryXXX(), the query will be executed
