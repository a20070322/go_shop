...
type CustomerQuery struct {
	...
	// eager-loading edges.
	withAddress   *CustomerAddressQuery
...
func (cq *CustomerQuery) QueryAddress() *CustomerAddressQuery {
	query := &CustomerAddressQuery{config: cq.config}
...
func (cq *CustomerQuery) QueryOrderInfo() *OrderInfoQuery {
...
func (cq *CustomerQuery) WithAddress(opts ...func(*CustomerAddressQuery)) *CustomerQuery {
	query := &CustomerAddressQuery{config: cq.config}
...
func (cq *CustomerQuery) WithOrderInfo(opts ...func(*OrderInfoQuery)) *CustomerQuery {
	query := &OrderInfoQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withAddress; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Address = []*CustomerAddress{}
		}
		query.withFKs = true
	...
	if query := cq.withOrderInfo; query != nil {
		...
		query.Where(predicate.OrderInfo(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.OrderInfoColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.customer_order_info
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_order_info" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_order_info" returned %v for node %v`, *fk, n.ID)
			}
