...
func GetTokenId(ctx *gin.Context) (int, error) {
	if cUid, uidBool := ctx.Get("uid"); uidBool == true {
		uid := cUid.(int)
		return uid, nil
	} else {
		return 0, errors.New("未找到此用户")
	}
}
...
func SetCatChToken(claims *Claims, token string, timer time.Duration) error {
	...
	_, err := global.Rdb.Set(context.Background(), str+"token_"+claims.UserGroup+"_"+strconv.Itoa(claims.UserID), token, timer).Result()
	return err
}
...
func CheckTokenCatch(claims *Claims, token string, IsRefreshToken bool) (bool, error) {
	...
	catchToken, err := global.Rdb.Get(context.Background(), str+"token_"+claims.UserGroup+"_"+strconv.Itoa(claims.UserID)).Result()
	if err != nil {
		return false, err
	}
